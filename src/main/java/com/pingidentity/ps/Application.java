...
public class Application {
    ...
    private String authcode(final HttpServletRequest request, final HttpServletResponse response, final Model model)
        ...
        redirectIfLoginRequired(request, response, env);
    ...
    private String login(final HttpServletRequest request) throws IOException {
        ...
        final OAuth2Helper oauth2Helper = new OAuth2Helper(env.getProperty("ping.federate.token.endpoing"));
        String redirectUrl = "/";

        System.out.println("=== Code: " + authCode);

        final String clientId = env.getProperty("ping.federate.client.id");
        final String clientSecret = env.getProperty("ping.federate.client.secret");
        final String redirectUri = env.getProperty("ping.federate.redirect.url");
        final String scopes = env.getProperty("ping.federate.scopes").replace(" ", "%20");
     ...
     */
    private String createRedirectUrl(final HttpServletRequest request, Environment env) throws MalformedURLException {
        final String authorizationEndpoint = env.getProperty("ping.federate.token.endpoing");
        final String clientId = env.getProperty("ping.federate.client.id");
        final String adapterId = env.getProperty("ping.federate.adapter.id");
        final String scopes = env.getProperty("ping.federate.scopes").replace(" ", "%20");
        final String redirectUri = env.getProperty("ping.federate.redirect.url");
     ...
     * @throws IOException Signals that an I/O exception of some sort has occurred.
     */
    private void redirectIfLoginRequired(final HttpServletRequest request, final HttpServletResponse response, Environment env) throws IOException {
        if (isLoginRequired(request)) {
            // Redirect to our authorization endpoint to begin the login process
            final String redirectUrl = createRedirectUrl(request, env);
